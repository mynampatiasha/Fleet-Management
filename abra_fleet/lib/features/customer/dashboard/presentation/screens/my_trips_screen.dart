...
class _RosterCardState extends State<RosterCard> {
  ...
  // Load daily trips for this roster - Generate date-wise trips from roster date range
  Future<void> _loadDailyTrips() async {
    ...
    try {
      ...
      // Generate daily trips from roster date range
      List<Map<String, dynamic>> trips = _generateDailyTripsFromRoster();
      
      // Try to get real trip data from backend if available
      if (rosterId != null && rosterId.isNotEmpty) {
        try {
          final response = await BackendConnectionManager().apiService.get(
            '/api/customer/stats/daily-trips',
            queryParams: {'rosterId': rosterId.toString()}
          );
          
          if (response['success'] == true && response['data'] != null) {
            final List<dynamic> backendTripsData = response['data'];
            
            // Merge backend data with generated trips
            for (var backendTrip in backendTripsData) {
              final tripDate = DateTime.parse(backendTrip['date']);
              final tripIndex = trips.indexWhere((trip) => 
                trip['date'].year == tripDate.year &&
                trip['date'].month == tripDate.month &&
                trip['date'].day == tripDate.day
              );
              
              if (tripIndex != -1) {
                // Update existing trip with backend data
                trips[tripIndex] = {
                  ...trips[tripIndex],
                  'status': backendTrip['status'] ?? trips[tripIndex]['status'],
                  'distance': (backendTrip['distance'] ?? 0.0).toDouble(),
                  'driverName': backendTrip['driverName']?.toString() ?? trips[tripIndex]['driverName'],
                  'driverPhone': backendTrip['driverPhone']?.toString() ?? trips[tripIndex]['driverPhone'],
                  'vehicleNumber': backendTrip['vehicleNumber']?.toString() ?? trips[tripIndex]['vehicleNumber'],
                  'tripId': backendTrip['tripId']?.toString() ?? trips[tripIndex]['tripId'],
                  'pickupTime': backendTrip['pickupTime']?.toString() ?? trips[tripIndex]['pickupTime'],
                  'dropoffTime': backendTrip['dropoffTime']?.toString() ?? trips[tripIndex]['dropoffTime'],
                };
              }
            }
          }
        } catch (e) {
          print('Backend trip data not available, using generated trips: $e');
        }
      }
      ...
      print('Error loading daily trips: $e');
      if (mounted) {
        setState(() {
          _isLoadingTrips = false;
        });
  ...
  // Handle individual trip cancellation with backend API call
  Future<void> _cancelTrip(Map<String, dynamic> trip) async {
    // Check if trip can be cancelled
    if (trip['canCancel'] != true && !_canCancelTripForDate(trip['date'])) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('Are you sure you want to cancel the trip scheduled for:'),
              const SizedBox(height: 8),
              Container(
    ...
    if (confirmCancel == true) {
      try {
        ...
        // Call backend API to cancel the specific trip
        final rosterId = widget.roster['rosterId'] ?? widget.roster['id'];
        final tripDate = DateFormat('yyyy-MM-dd').format(trip['date']);
        ...
        if (response['success'] == true) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Trip for ${trip['dateString']} has been cancelled successfully'),
                backgroundColor: Colors.green,
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Trip for ${trip['dateString']} has been cancelled (offline mode)'),
              backgroundColor: Colors.orange,
              action: SnackBarAction(
                label: 'Undo',
                textColor: Colors.white,
  ...
  Future<void> _undoCancelTrip(Map<String, dynamic> trip) async {
    try {
      ...
      if (response['success'] == true) {
        setState(() {
          trip['status'] = _getTripStatusForDate(trip['date']);
          trip['canCancel'] = _canCancelTripForDate(trip['date']);
        });
        
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Trip for ${trip['dateString']} has been restored'),
              backgroundColor: Colors.blue,
            ),
          );
        }
      } else {
        throw Exception(response['message'] ?? 'Failed to restore trip');
      }
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Trip for ${trip['dateString']} has been restored (offline mode)'),
            backgroundColor: Colors.blue,
          ),
        );
      }
