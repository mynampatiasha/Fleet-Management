...
class _NewCustomerPageState extends State<NewCustomerPage> {
  ...
  // ============================================================================
  String selectedPaymentTerms = 'Immediate/COD';
  ...
  String selectedBillingFrequency = 'Per-trip';
  ...
  List<String> selectedInvoiceDeliveryMethods = ['Email'];
  String selectedInvoiceLanguage = 'English';
  String? selectedTaxRegistrationType;
  ...
  final List<String> invoiceDeliveryMethods = [
    'Email',
    'WhatsApp',
    'SMS',
    'Portal Download',
  ...
  final List<String> invoiceLanguages = [
    'English',
    'Hindi',
    'Tamil',
    'Telugu',
  ...
  final List<String> taxRegistrationTypes = [
    'Registered',
    'Unregistered',
    'Composition'
  ];
  ...
  // ============================================================================
  // SAVE CUSTOMER METHOD - INTEGRATED WITH SERVICE
  // ============================================================================
  ...
  Future<void> _saveCustomer() async {
    ...
    try {
      ...
      final customerData = BillingCustomersService.buildCustomerDataFromControllers(
        ...
        invoiceDeliveryMethods: selectedInvoiceDeliveryMethods,
        invoiceLanguage: selectedInvoiceLanguage,
        taxRegistrationType: selectedTaxRegistrationType,
      ...
      if (result['success'] == true) {
        ...
        final createdCustomerId = createdCustomer['_id']; // MongoDB ID
        ...
        // ============================================================================
        // SUCCESS - CLOSE AND SHOW MESSAGE
        // ============================================================================
        Navigator.pop(context); // Close loading dialog
        setState(() => isSaving = false);
  ...
  Future<void> _loadCustomerData(String customerId) async {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        setState(() {
          // Section 1: Basic Information
          selectedCustomerType = customer['customerType'] ?? 'Individual';
          _customerDisplayNameController.text = customer['customerDisplayName'] ?? '';
          _primaryContactPersonController.text = customer['primaryContactPerson'] ?? '';
          _primaryEmailController.text = customer['primaryEmail'] ?? '';
          _primaryPhoneController.text = customer['primaryPhone'] ?? '';
          ...
          selectedCountry = customer['country'] ?? 'India';
          
          // Section 2: Company Details
          _companyRegistrationController.text = customer['companyRegistration'] ?? '';
          _panNumberController.text = customer['panNumber'] ?? '';
          _gstNumberController.text = customer['gstNumber'] ?? '';
          _tanNumberController.text = customer['tanNumber'] ?? '';
          selectedIndustryType = customer['industryType'];
          ...
          // Section 4: Categorization
          selectedCustomerStatus = customer['customerStatus'] ?? 'Active';
          _reasonForBlockingController.text = customer['reasonForBlocking'] ?? '';
          selectedCustomerTier = customer['customerTier'];
          selectedSalesTerritory = customer['salesTerritory'] ?? 'Bangalore';
          selectedTags = List<String>.from(customer['tags'] ?? []);
          
          // Section 5: Rate Card & Pricing
          selectedRateCard = customer['rateCard'];
          selectedContractType = customer['contractType'];
          _contractStartDateController.text = customer['contractStartDate'] ?? '';
          _contractEndDateController.text = customer['contractEndDate'] ?? '';
          autoRenewal = customer['autoRenewal'] ?? false;
          _renewalNoticePeriodController.text = customer['renewalNoticePeriod'] ?? '';
          
          // Vendor-specific
          selectedVendorCommissionType = customer['vendorCommissionType'];
          _commissionRateController.text = customer['commissionRate'] ?? '';
          _fixedAmountPerTripController.text = customer['fixedAmountPerTrip'] ?? '';
          _revenueShareController.text = customer['revenueShare'] ?? '';
          _minimumGuaranteeController.text = customer['minimumGuarantee'] ?? '';
          selectedPaymentCycle = customer['paymentCycle'];
          selectedVehicleTypesProvided = List<String>.from(customer['vehicleTypesProvided'] ?? []);
          _numberOfVehiclesProvidedController.text = customer['numberOfVehiclesProvided'] ?? '';
          
          // Section 6: Payment Terms & Credit
          selectedPaymentTerms = customer['paymentTerms'] ?? 'Immediate/COD';
          selectedPreferredPaymentMethod = customer['preferredPaymentMethod'];
          _creditLimitController.text = customer['creditLimit'] ?? '';
          _securityDepositController.text = customer['securityDeposit'] ?? '';
          selectedSecurityDepositStatus = customer['securityDepositStatus'];
          blockBookingIfCreditExceeded = customer['blockBookingIfCreditExceeded'] ?? false;
          selectedBillingFrequency = customer['billingFrequency'] ?? 'Per-trip';
          
          // Section 7: Billing Preferences
          _billingEmailController.text = customer['billingEmail'] ?? '';
          _billingAddressController.text = customer['billingAddress'] ?? '';
          sameAsPrimaryAddress = customer['sameAsPrimaryAddress'] ?? true;
          poNumberRequired = customer['poNumberRequired'] ?? false;
          selectedInvoiceDeliveryMethods = List<String>.from(customer['invoiceDeliveryMethods'] ?? ['Email']);
          selectedInvoiceLanguage = customer['invoiceLanguage'] ?? 'English';
          selectedTaxRegistrationType = customer['taxRegistrationType'];
          
          // Section 8: Vendor-Specific Details
          _vendorVehiclesAvailableController.text = customer['vendorVehiclesAvailable'] ?? '';
          vendorVehicleTypes = List<String>.from(customer['vendorVehicleTypes'] ?? []);
          _vendorAgreementStartController.text = customer['vendorAgreementStart'] ?? '';
          _vendorAgreementEndController.text = customer['vendorAgreementEnd'] ?? '';
          vendorPerformanceRating = customer['vendorPerformanceRating']?.toDouble() ?? 0.0;
          _insuranceValidUntilController.text = customer['insuranceValidUntil'] ?? '';
          ...
          _upiIdController.text = customer['upiId'] ?? '';
          
          // Section 10: Additional Information
          _internalNotesController.text = customer['internalNotes'] ?? '';
          _customerInstructionsController.text = customer['customerInstructions'] ?? '';
          _specialRequirementsController.text = customer['specialRequirements'] ?? '';
          ...
          createdDate = customer['createdDate'] != null 
              ? DateTime.parse(customer['createdDate']) 
              : null;
          lastModifiedBy = customer['lastModifiedBy'] ?? '';
          lastModifiedDate = customer['lastModifiedDate'] != null 
              ? DateTime.parse(customer['lastModifiedDate']) 
              : null;
          totalRevenueGenerated = customer['totalRevenueGenerated']?.toDouble() ?? 0.0;
          ...
          : Form(
              ...
              child: Row(
                ...
                children: [
                  ...
                  // Right sidebar - Summary & Audit Trail
                  Container(
                    width: 350,
                    color: Colors.white,
                    child: SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (!isVendor) ...[
            ...
            // Vendor-specific rate card (commission-based)
            const Text(
    ...
    return _buildSection(
      ...
      child: Column(
        children: [
          ...
          // Block Booking if Credit Exceeded (not for vendors)
          if (!isVendor) ...[
            CheckboxListTile(
              title: const Text('Block Booking if Credit Exceeded', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
              subtitle: const Text('Prevent new bookings when credit limit is reached', style: TextStyle(fontSize: 12)),
              value: blockBookingIfCreditExceeded,
              onChanged: (value) {
                setState(() => blockBookingIfCreditExceeded = value!);
              },
              contentPadding: EdgeInsets.zero,
              controlAffinity: ListTileControlAffinity.leading,
            ),
    ...
    return _buildSection(
      ...
      child: Column(
        children: [
          ...
          // PO Number Required (for organizations)
          if (selectedCustomerType == 'Organization') ...[
            CheckboxListTile(
              title: const Text('PO Number Required', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
              subtitle: const Text('Require purchase order number for invoicing', style: TextStyle(fontSize: 12)),
              value: poNumberRequired,
              onChanged: (value) {
                setState(() => poNumberRequired = value!);
              },
              contentPadding: EdgeInsets.zero,
              controlAffinity: ListTileControlAffinity.leading,
            ),
          ...
          _buildOptionalField(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...invoiceDeliveryMethods.map((method) {
                  return CheckboxListTile(
                    title: Text(method, style: const TextStyle(fontSize: 14)),
                    value: selectedInvoiceDeliveryMethods.contains(method),
                    onChanged: (selected) {
                      setState(() {
                        if (selected == true) {
                          selectedInvoiceDeliveryMethods.add(method);
                        } else {
                          selectedInvoiceDeliveryMethods.remove(method);
                        }
                      });
                    },
                    contentPadding: EdgeInsets.zero,
          ...
          _buildOptionalField(
            ...
            child: DropdownButtonFormField<String>(
              ...
              decoration: _inputDecoration(),
              items: invoiceLanguages.map((language) {
                return DropdownMenuItem(value: language, child: Text(language));
              }).toList(),
              onChanged: (value) {
                setState(() => selectedInvoiceLanguage = value!);
              },
            ),
          ),
          const SizedBox(height: 20),
          ...
          _buildOptionalField(
            ...
            child: DropdownButtonFormField<String>(
              value: selectedTaxRegistrationType,
              decoration: _inputDecoration(),
              hint: const Text('Select tax registration type'),
              items: taxRegistrationTypes.map((type) {
                return DropdownMenuItem(value: type, child: Text(type));
              }).toList(),
              onChanged: (value) {
                setState(() => selectedTaxRegistrationType = value);
              },
            ),
          ),
        ],
