  ...
  Future<void> _performAdvancedRouteOptimization(int count) async {
    ...
    try {
      ...
      if (errorStr.contains('ApiException:')) {
        ...
        // Don't set generic "Backend Error" title - let specific checks below set user-friendly titles
      ...
      if (errorStr.contains('No suitable vehicle') || errorStr.contains('No compatible vehicles')) {
      ...
      } else if (errorStr.contains('cannot reach next shift') || errorStr.contains('FEASIBILITY_FAILED')) {
  ...
  try {
    ...
    final routeData = route.map((stop) {
      ...
      return {
        ...
        'sequence': sequence,
        'pickupTime': DateFormat('HH:mm').format(stop['eta'] as DateTime),
    ...
    // ✅ CRITICAL FIX: Force refresh to remove assigned rosters from list
    ...
    // 🔥 FIX: Extract success count with fallback logic
    ...
    if (result['notifications'] != null) {
      ...
      if (notifs['customers'] != null) {
        debugPrint('   - customers field type: ${notifs['customers'].runtimeType}');
        debugPrint('   - customers value: ${notifs['customers']}');
        
        if (notifs['customers'] is Map) {
          customerNotifs = notifs['customers']['sent'] ?? 0;
          debugPrint('   ✅ Extracted from Map: $customerNotifs');
        } else if (notifs['customers'] is int) {
          customerNotifs = notifs['customers'];
          debugPrint('   ✅ Used int directly: $customerNotifs');
        }
      }
      
      // Extract driver notifications safely
      if (notifs['driver'] != null) {
        debugPrint('   - driver field type: ${notifs['driver'].runtimeType}');
        debugPrint('   - driver value: ${notifs['driver']}');
        
        if (notifs['driver'] is Map) {
          driverNotifs = notifs['driver']['sent'] ?? 0;
          debugPrint('   ✅ Extracted from Map: $driverNotifs');
        } else if (notifs['driver'] is int) {
          driverNotifs = notifs['driver'];
          debugPrint('   ✅ Used int directly: $driverNotifs');
        }
      }
    } else {
      debugPrint('⚠️ No notifications field in response');
    }
    ...
    if (successCount > 0) {
      ...
      scaffoldMessenger.showSnackBar(
        SnackBar(
          content: const Text(
            '⚠️ Assignment completed but 0 customers were assigned\n'
            'Please check if the rosters were already assigned or contact support',
            style: TextStyle(fontSize: 14),
          ),
    ...
    // ✅ CRITICAL FIX: Force refresh even on failure to remove already assigned rosters
    debugPrint('🔄 (Error Recovery) Reloading pending rosters with FORCE REFRESH...');
    _loadPendingRosters(forceRefresh: true).then((_) {
      debugPrint('✅ (Error Recovery) Rosters refreshed - stale data removed');
    });
    ...
    if (errorStr.contains('already assigned')) {
      errorTitle = '⚠️ Some Customers Already Assigned';
      actionGuidance = 'Solutions:\n'
          '• The list has been refreshed automatically\n'
          '• Already-assigned customers have been removed\n'
        ...
        builder: (context) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.pop(context);
                // Retry optimization with refreshed list
...
setState(() => _isOptimizing = false);
                          ...
                          ElevatedButton.icon(
                            onPressed: selectedVehicle == null
                                ...
                                : () {
                                    ...
                                    Navigator.of(context).pop();
                                    _confirmManualAssignment(customers, selectedVehicle!);
                                  },
  ...
  Future<void> _confirmManualAssignment(
    ...
    try {
      ...
      // Extract roster IDs
      final rosterIds = customers.map((customer) {
        return customer['id']?.toString() ?? customer['_id']?.toString() ?? '';
      }).where((id) => id.isNotEmpty).toList();

      debugPrint('📝 Roster IDs extracted: ${rosterIds.length}');
      for (int i = 0; i < rosterIds.length; i++) {
        final customerName = _safeStringExtract(customers[i]['customerName']) != '' ?
                           _safeStringExtract(customers[i]['customerName']) :
                           (_safeNestedAccess(customers[i], 'employeeDetails', 'name') != '' ?
                           _safeNestedAccess(customers[i], 'employeeDetails', 'name') :
                           'Unknown');
        debugPrint('   ${i + 1}. $customerName (${rosterIds[i]})');
      }
      debugPrint('-'*80);

      debugPrint('🚀 CALLING BACKEND API: assignRostersToVehicle() [MANUAL MODE]');
      debugPrint('   - Vehicle ID: $vehicleId');
      debugPrint('   - Roster count: ${rosterIds.length}');
      debugPrint('   - Mode: manual');

      // Call backend API to assign rosters to vehicle
      final result = await widget.rosterService.assignRostersToVehicle(
        vehicleId: vehicleId,
        rosterIds: rosterIds,
        routeDetails: {
          'mode': 'manual',
      ...
      // Reload rosters
      debugPrint('🔄 Reloading pending rosters...');
     await _loadPendingRosters(forceRefresh: true);
      debugPrint('✅ Rosters reloaded');
      ...
      debugPrint('\n' + '❌'*40);
      debugPrint('MANUAL ASSIGNMENT FAILED');
      debugPrint('❌'*40);
      debugPrint('Error: $e');
      debugPrint('Stack trace:');
      debugPrint(stackTrace.toString());
      ...
      debugPrint('📱 Showing error message to user');
      _showSnackBar('Assignment failed: ${e.toString()}', backgroundColor: Colors.red);
    }
  }
  ...
  List<Map<String, dynamic>> _removeDuplicateRosters(List<Map<String, dynamic>> rosters) {
    ...
    for (var roster in rosters) {
      try {
        ...
        final uniqueKey = '$email|$startDate|$startTime|$rosterType|$officeLocation';
        
        debugPrint('🔍 Processing roster - Email: $email, Key: $uniqueKey');
        
        if (!seenKeys.contains(uniqueKey) && email.isNotEmpty) {
          seenKeys.add(uniqueKey);
          uniqueRosters.add(roster);
          debugPrint('✅ Added unique roster for: $email');
        } else {
          duplicatesFound++;
          debugPrint('⚠️ Duplicate found for: $email (Key: $uniqueKey)');
        }
      } catch (e) {
  ...
  void _applyFilters() {
    ...
    if (_selectedDateRange != null) {
      debugPrint('📅 Applying date range filter: ${_selectedDateRange!.start} to ${_selectedDateRange!.end}');
      int beforeCount = tempResults.length;
      tempResults = tempResults.where((roster) {
        final dateStr = roster['startDate']?.toString();
      ...
      debugPrint('📅 Date range filter: ${beforeCount} → ${tempResults.length} rosters');
    }
...
//     try {
//       debugPrint('🚗 Loading available drivers...');
      
//       // This would typically call a driver service
//       // For now, we'll use a placeholder implementation
//       setState(() {
//         _availableDrivers = []; // Placeholder - implement actual driver loading
//       });
...
//   /// Load pending rosters from the service
//   Future<void> _loadPendingRosters() async {
//     try {
//       setState(() {
//         _isLoading = true;
//         _errorMessage = null;
...
//       final response = await widget.rosterService.getPendingRosters();
      
//       if (response['success'] == true) {
//         final rosters = List<Map<String, dynamic>>.from(response['data'] ?? []);
        
//         setState(() {
//           _allPendingRosters = rosters;
//           _filteredRosters = rosters;
//           _isLoading = false;
//         });
        
//         debugPrint('✅ Loaded ${rosters.length} pending rosters');
//         _updateOrganizationFilter();
//       } else {
...
//   void _updateOrganizationFilter() {
//     final organizations = <String>{'All Organizations'};
    
//     for (var roster in _allPendingRosters) {
//       final org = roster['organization'] ?? roster['officeLocation'] ?? 'Unknown';
//       if (org.toString().isNotEmpty) {
//         organizations.add(org.toString());
//       }
//     }
    
//     setState(() {
//       _availableOrganizations = organizations.toList();
//     });
//   }
...
//   /// Calculate priority for a roster
//   Map<String, dynamic> _calculatePriority(Map<String, dynamic> roster) {
//     // Implementation for calculating roster priority
//     // This is a placeholder - implement actual priority logic
//     return {
//       'level': 'normal',
//       'score': 0,
...
//   /// Build individual roster card
//   Widget _buildRosterCard(Map<String, dynamic> roster) {
//     final customerName = roster['customerName'] ?? 
//                        roster['employeeDetails']?['name'] ?? 
//                        'Unknown Customer';
//     final rosterType = roster['rosterType'] ?? 'Unknown';
//     final organization = roster['organization'] ?? roster['officeLocation'] ?? 'Unknown';
...
//           children: [
//             const SizedBox(height: 4),
//             Text('Type: ${_formatRosterType(rosterType)}'),
...
//           debugPrint('Bulk assign ${_selectedRosterIds.length} rosters');
//         },
//         backgroundColor: _themeColor,
//         icon: const Icon(Icons.assignment_turned_in),
//         label: Text('Assign ${_selectedRosterIds.length}'),
//       ),
//     );
//   }
// }
...
// // ==========================================
// // ROUTE OPTIMIZATION DIALOG WIDGET
// // ==========================================
...
//   Widget build(BuildContext context) {
//     final route = routePlan['route'] as List<Map<String, dynamic>>;
//     final vehicleName = routePlan['vehicleName'] ?? 'Unknown Vehicle';
//     final driverName = routePlan['driverName'] ?? 'Unknown Driver';
//     final totalDistance = routePlan['totalDistance'] ?? 0.0;
//     final totalTime = routePlan['totalTime'] ?? 0;
...
//                         final customerName = stop['customerName'] ?? 'Unknown';
//                         final location = stop['location'] ?? 'Unknown Location';
//                         final eta = stop['eta'] as DateTime?;
//                         final distance = stop['distanceFromPrevious'] ?? 0.0;
...
//                                         const SizedBox(height: 4),
//                                         Text(
//                                           'ETA: ${DateFormat('h:mm a').format(eta)}',
//                                           style: TextStyle(
//                                             fontSize: 12,
//                                             color: Colors.green.shade700,
//                                             fontWeight: FontWeight.w600,
...
//                                         const SizedBox(height: 4),
//                                         Text(
//                                           '${distance.toStringAsFixed(1)} km from previous',
//                                           style: const TextStyle(
//                                             fontSize: 11,
//                                             color: Colors.grey,
//                                           ),
