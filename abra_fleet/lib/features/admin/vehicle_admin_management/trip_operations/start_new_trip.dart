...
class _StartNewTripPageState extends State<StartNewTripPage> {
      ...
      if (response['success'] == true) {
        ...
        setState(() {
          _vehicles = vehiclesData
            ...
            // Handle different capacity field formats safely
  ...
  String _getVehicleDisplayText(Map<String, dynamic> vehicle) {
    final registration = vehicle['registration'] ?? 'N/A';
    final type = vehicle['type'] ?? 'N/A';
    final driverName = vehicle['driverName'] ?? 'No Driver';
    final status = vehicle['status'] ?? 'ACTIVE';
    final statusDisplay = status.isNotEmpty 
        ? status[0].toUpperCase() + status.substring(1).toLowerCase()
        : status;
    return '$registration - $type ($statusDisplay) - Driver: $driverName';
  }
    ...
    try {
      ...
      final tripData = {
        ...
        'startPoint': {
          ...
          'address': 'Pickup Location', // You can enhance this with reverse geocoding
        },
        'endPoint': {
          'latitude': _endPoint!.latitude,
          'longitude': _endPoint!.longitude,
          'address': 'Drop Location', // You can enhance this with reverse geocoding
        },
      ...
      // Use ApiService createTrip method
            ...
            SnackBar(
              content: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  const Text('✅ Driver notified  ✅ Admin notified'),
                ],
              ),
      ...
      if (e.toString().contains('ApiException:')) {
        try {
          // Try to parse the ApiException JSON
          ...
          DropdownButtonFormField<String>(
            ...
            items: _vehicles.map((vehicle) {
              return DropdownMenuItem<String>(
                value: vehicle['id'],
                child: Text(_getVehicleDisplayText(vehicle)),
              );
            }).toList(),
            onChanged: (value) {
              setState(() {
                            ...
                            children: [
                              ...
                              Text(
                                _startPoint != null && _endPoint != null
                                    ...
                                    : 'Tap to select start and end points',
                                style: TextStyle(
                                  fontSize: 14,
  ...
  Widget _buildRoutePoint(String label, IconData icon, Color color, LatLng point) {
    return Container(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoRow('Distance:', '${distance.toStringAsFixed(2)} km'),
          _buildInfoRow('Estimated Time:', '$estimatedTime minutes'),
          _buildInfoRow('Start:', '${_startPoint!.latitude.toStringAsFixed(4)}, ${_startPoint!.longitude.toStringAsFixed(4)}'),
          _buildInfoRow('End:', '${_endPoint!.latitude.toStringAsFixed(4)}, ${_endPoint!.longitude.toStringAsFixed(4)}'),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(height: 16),
          _buildInfoRow('Vehicle ID:', vehicle['vehicleId'] ?? 'N/A'),
          _buildInfoRow('Registration:', vehicle['registration'] ?? 'N/A'),
          _buildInfoRow('Type:', vehicle['type'] ?? 'N/A'),
          _buildInfoRow('Model:', vehicle['model'] ?? 'N/A'),
          _buildInfoRow('Seating Capacity:', vehicle['seatingCapacity'] ?? 'N/A'),
          const SizedBox(height: 8),
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                _buildInfoRow('Driver Name:', vehicle['driverName'] ?? 'N/A'),
                if (vehicle['driverPhone'] != null && vehicle['driverPhone'].toString().isNotEmpty)
                  _buildInfoRow('Driver Phone:', vehicle['driverPhone']),
              ],
            ),
          ),
        ],
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  _startPoint != null && _endPoint != null && _selectedVehicleId != null
                      ? 'Route looks good! Estimated travel time: ${(_calculateRouteDistance() / 40 * 60).round()} minutes with assigned driver.'
                      : _vehicles.isEmpty
                          ? 'Assign drivers to vehicles first, then select route points to get AI-powered suggestions.'
                          : 'Select a vehicle with assigned driver and route points to get AI-powered suggestions.',
                  style: const TextStyle(
                    color: Color(0xFF01579B),
                  ),
                ),
