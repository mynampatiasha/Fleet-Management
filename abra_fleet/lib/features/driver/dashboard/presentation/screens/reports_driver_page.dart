...
import 'dart:io' show File;  // Only import File class, not the whole dart:io
...
class _ReportsDriverPageState extends State<ReportsDriverPage> {
  ...
  Future<void> _handleGenerateReport(String type) async {
    // Show centered dialog instead of snackbar
    showDialog(
      context: context,
  ...
  // Changed to centered dialog instead of bottom sheet
  ...
  /// Cross-platform download and open
  /// Cross-platform download and open
Future<void> _downloadAndOpenFile(String reportId) async {
  ...
  try {
    if (kIsWeb) {
      ...
      // Mobile: Download and open file
      final filePath = await _reportsService.downloadReport(reportId);
      Navigator.pop(context);
      
      // Open the downloaded file using FileDownloadHelper
      try {
  ...
  /// Cross-platform download and share
  /// Cross-platform download and share
...
Future<void> _downloadFileWeb(String reportId, {bool shouldShare = false}) async {
      ...
      builder: (context, snapshot) {
        ...
        return Column(
          children: [
            ...
            const SizedBox(height: 16),
            _buildTripList(tripsResponse.trips),
          ],
        );
      },
    );
  ...
  Widget _buildTripList(List<TripInfo> trips) {
      ...
      return _buildCard(
        ...
        child: const Center(
          child: Padding(
            ...
            child: Text("No trips found for the selected period."),
          ),
        ),
      );
    }
    ...
    return _buildCard(
      title: 'Trips (${trips.length})',
      icon: '🚐',
      child: Column(
        children: [
          // Add refresh button for trips
          Row(
                ...
                child: ListTile(
                  leading: CircleAvatar(
                    backgroundColor: trip.status == 'completed'
                        ? Colors.green
                        : (trip.status == 'cancelled' ? kDangerColor : kWarningColor),
                    child: Icon(
                        trip.status == 'completed' ? Icons.check : Icons.local_shipping,
                        color: Colors.white),
                  ),
                  title: Text(trip.customerName,
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text(
                    '${trip.distance.toStringAsFixed(1)} km - ${DateFormat.yMMMd().format(trip.startTime!)}',
                  ),
                  trailing: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(trip.status.capitalize(),
                          style: TextStyle(
                              color: trip.status == 'completed' ? Colors.green : kSecondaryTextColor,
    ...
    builder: (context, snapshot) {
      ...
      return _buildCard(
        ...
        child: GridView.count(
          ...
          crossAxisSpacing: 12,
          mainAxisSpacing: 12,
          childAspectRatio: 1.4, // Adjusted for better mobile display
          padding: const EdgeInsets.symmetric(vertical: 8),
          children: [
            _buildStatItem(
              number: summary.totalTrips.toString(),
              label: 'Total Trips',
            ),
            _buildStatItem(
            ...
            _buildStatItem(
              number: '${summary.onTimePercentage}%',
              label: 'On-Time %',
            ),
            _buildStatItem(
              number: '${summary.totalKm.toStringAsFixed(1)} km',
    ...
    return FutureBuilder<DailyAnalytics>(
      ...
      builder: (context, snapshot) {
        ...
        return _buildCard(
          ...
          child: Column(
            children: [
              _buildReadOnlyField(label: 'Working Hours Today', value: analytics.workingHours),
              const SizedBox(height: 12),
              _buildReadOnlyField(label: 'Fuel Efficiency', value: analytics.fuelEfficiency),
              const SizedBox(height: 12),
              _buildReadOnlyField(label: 'Trips Today', value: analytics.tripsToday.toString()),
              const SizedBox(height: 12),
              _buildReadOnlyField(label: 'Distance Today', value: '${analytics.distanceToday} KM'),
            ],
          ),
        );
      },
  ...
  Widget _buildTripReportsCard() {
    return _buildCard(
      ...
      child: Column(
        children: [
          // Reduced width buttons with center alignment
          Center(
  ...
  Widget _buildStatItem({required String number, required String label}) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            number,
            style: const TextStyle(
                fontSize: 26.0, fontWeight: FontWeight.bold, color: kPrimaryColor),
          ),
          const SizedBox(height: 3.0),
          Text(
            label,
            style: const TextStyle(fontSize: 12.0, color: kSecondaryTextColor),
    ...
    return Column(
      ...
      children: [
        Text(
          label,
          style: const TextStyle(
              fontWeight: FontWeight.w600, color: kPrimaryTextColor, fontSize: 14),
        ),
