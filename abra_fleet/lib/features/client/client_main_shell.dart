...
class _ClientMainShellState extends State<ClientMainShell> {
  ...
  Future<void> _setupUnreadCountListener() async {
  final prefs = await SharedPreferences.getInstance();
  ...
  debugPrint('🔔 CLIENT SHELL: Setting up unread count listener for $_userId');
  
  // Poll notifications from backend API every 10 seconds
  Stream.periodic(const Duration(seconds: 10)).asyncMap((_) async {
    ...
    if (response['success'] == true && response['count'] != null) {
      final int unreadCount = response['count'] as int;
      debugPrint('📊 CLIENT SHELL: Unread count: $unreadCount');
