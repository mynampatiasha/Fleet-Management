// lib/features/hrm_feedback/presentation/screens/hrm_notice_board_screen.dart
...
class HrmNoticeBoardScreen extends StatefulWidget {
  const HrmNoticeBoardScreen({super.key});

  @override
  State<HrmNoticeBoardScreen> createState() => _HrmNoticeBoardScreenState();
}

class _HrmNoticeBoardScreenState extends State<HrmNoticeBoardScreen> with ErrorHandlerMixin {
  ...
  Future<void> _fetchLeaveRequests() async {
    ...
    try {
      ...
      if (response['success'] == true && mounted) {
        setState(() {
          _leaveRequests = List<Map<String, dynamic>>.from(response['data'] ?? []);
        });
      }
    } catch (e) {
  ...
  void _exportToCSV() {
    ...
    try {
      ...
      for (var leave in _leaveRequests) {
        ...
        final employeeName = leave['employee_name'] ?? 'Unknown';
        final startDate = leave['start_date'] != null 
            ? DateFormat('yyyy-MM-dd').format(DateTime.parse(leave['start_date']))
            : '';
        final endDate = leave['end_date'] != null 
            ? DateFormat('yyyy-MM-dd').format(DateTime.parse(leave['end_date']))
            : '';
