...
// ✅ HELPER: Server-Side Geocoding
// If the CSV has "Koramangala, Bangalore", this converts it to {lat, lng}
async function geocodeAddress(address) {
  if (!address || typeof address !== 'string') return null;

  try {
    const encodedAddress = encodeURIComponent(address);
    // Using OpenStreetMap (Free, no API key required, but requires User-Agent)
...
// @desc    Group pending rosters by matching criteria (organization, time, location)
// @access  Private (Admin)
// ============================================================================
// ENHANCED SMART GROUPING ENDPOINT
// File: routes/roster_router.js (REPLACE existing /admin/group-similar endpoint)
// ============================================================================
// 
// This implementation adds INDUSTRY-STANDARD features:
// ✅ Time window tolerance (±15 minutes)
// ✅ Geographic clustering (5km radius)
// ✅ Vehicle capacity pre-validation
// ✅ Weekday matching
// ✅ String normalization
// ✅ Smart group splitting based on available capacity
//
// ============================================================================



// Replace your /admin/group-similar route with this DEBUG version

// ============================================================================
// 🔧 ENHANCED GROUPING FIX - WITH FULL EMAIL DOMAIN MATCHING
// File: Replace /admin/group-similar route in roster_router.js
// Issue: Group by FULL domain (e.g., @wipro.com) instead of company name
// ============================================================================

router.post('/admin/group-similar', verifyToken, async (req, res) => {
  try {
    console.log('\n' + '🔍'.repeat(80));
    console.log('ENHANCED SMART GROUPING - WITH EMAIL DOMAIN MATCHING');
    console.log('🔍'.repeat(80));
    ...
    if (pendingRosters.length === 0) {
      return res.json({
        ...
        data: { groups: [], totalRosters: 0 }
      });
    }

    // ========================================================================
    // HELPER: Extract FULL domain from email (@wipro.com, @infosys.com, etc.)
    // ========================================================================
    ...
    if (pendingRosters.length > 0) {
      ...
      console.log('   weeklyOffDays:', firstRoster.weeklyOffDays);
    }

    // ========================================================================
    // GROUPING LOGIC WITH EMAIL DOMAIN
    // ========================================================================
    ...
    for (const roster of pendingRosters) {
      // ✅ Extract FULL email domain (e.g., @wipro.com)
      ...
      if (!groups[groupKey]) {
        groups[groupKey] = {
          groupKey,
          emailDomain: emailDomain, // Store full domain
          loginTime,
          logoutTime,
    ...
    // Show all groups (including single-member ones for debugging)
    Object.values(groups).forEach((group, idx) => {
      ...
      console.log(`   - Weekdays: ${Array.isArray(group.weekdays) && group.weekdays.length > 0 ? group.weekdays.join(', ') : 'None'}`);
      console.log(`   - Employees: ${group.employeeCount}`);
      console.log(`   - Members: ${group.employees.map(e => e.name).join(', ')}`);
    });


    // Filter groups with 1+ members (accept all groups)
    const validGroups = Object.values(groups).filter(g => g.employeeCount >= 1);

    console.log(`\n📊 Total valid groups: ${validGroups.length}`);
    ...
    res.json({
      success: true,
      message: `Found ${validGroups.length} groups`,
      data: {
        groups: validGroups.map(group => ({
          groupId: group.groupKey,
          emailDomain: group.emailDomain, // Full domain like @wipro.com
          organization: group.emailDomain.replace('@', ''), // wipro.com (for display)
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // ✅ Server-side geocoding (if coordinates missing)
      if ((!officeLocationCoordinates || !officeLocationCoordinates.latitude) && officeLocation) {
        ...
        if (coords) {
          ...
          return res.status(400).json({
            success: false,
            message: `Failed to geocode office location: "${officeLocation}". Please provide a valid, complete address (e.g., "Koramangala 5th Block, Bangalore").`
          });
        }
      }
      ...
      if ((rosterType === 'logout' || rosterType === 'both') &&
        ...
        if (coords) {
          ...
          return res.status(400).json({
            success: false,
            message: `Failed to geocode drop location: "${logoutDropAddress}". Please provide a valid, complete address (e.g., "Abra Fleet Bangalore Office").`
          });
        }
      }
...
router.post('/admin/assign',
  ...
  async (req, res) => {
    console.log('\n' + '='.repeat(80));
    console.log('🎯 ROSTER ASSIGNMENT (VIA ONESIGNAL SERVICE)');
    console.log('='.repeat(80));
    ...
    try {
      ...
      console.log('\n🚗 Step 2: Fetching driver and vehicle details...');
      // Get driver and vehicle details
      const [driver, vehicle] = await Promise.all([
        req.db.collection('drivers').findOne({
          $or: [
        ...
        req.db.collection('vehicles').findOne({
          $or: [
      ...
      // ========================================================================
      // Step 4: Update roster assignment in MongoDB
      // ========================================================================
      console.log('\n💾 Step 4: Updating roster assignment in MongoDB...');
      const updatedRoster = await Roster.assignDriverVehicle(
        rosterId, driverId, vehicleId, req.user.userId
      );

      if (!updatedRoster) {
        return res.status(404).json({ success: false, message: 'Roster not found or already assigned' });
      }

      // ========================================================================
      // Step 5: Store Distance Data
      // ========================================================================
      if (distanceData && !distanceData.error) {
        try {
          await req.db.collection('rosters').updateOne(
            { _id: new ObjectId(rosterId) },
            { $set: { distanceData: { ...distanceData, calculatedBy: req.user.userId } } }
          );
        } catch (e) { console.log('Distance save failed'); }
      }

      // ========================================================================
      // Step 6: Create customer notification (VIA ONESIGNAL SERVICE)
      // ========================================================================
      console.log('\n📧 Step 6: Creating customer notification via OneSignal...');
      try {
        let notificationBody = `Your roster has been assigned. Driver: ${driver.name}, Vehicle: ${vehicle.registrationNumber}`;
        if (distanceData && !distanceData.error && distanceData.totalDistanceKm > 0) {
          notificationBody += `\nEst. Distance: ${distanceData.totalDistanceKm} km`;
        }

        // Send via Service (Triggers OneSignal + WebSocket + DB)
        await NotificationService.sendRealTimeNotification(
          'customer',       // User Type
          roster.userId,    // User ID (Target)
          {                 // Notification Object
        ...
        console.log('✅ Customer notification sent via Service');
      } catch (notificationError) {
        console.error('❌ Failed to send customer notification:', notificationError);
      }

      // ========================================================================
      // Step 7: Create driver notification (VIA ONESIGNAL SERVICE)
      // ========================================================================
      console.log('\n📧 Step 7: Creating driver notification via OneSignal...');
      try {
        const driverUser = await req.db.collection('users').findOne({
          $or: [{ driverId: driver._id }, { email: driver.email }]
        });

        if (driverUser && driverUser.firebaseUid) {
          let driverBody = `Customer: ${roster.customerName}\nLocation: ${roster.officeLocation}`;
          
          // Send via Service
          await NotificationService.sendRealTimeNotification(
            'driver',               // User Type
            driverUser.firebaseUid, // User ID (Target)
            {                       // Notification Object
              type: 'roster_assigned',
          ...
          console.log('✅ Driver notification sent via Service');
        } else {
          console.log('⚠️ Driver User ID not found, skipping push notification');
        }
      } catch (driverError) {
        console.error('❌ Failed to send driver notification:', driverError);
      }

      // ========================================================================
      // Final Response
      // ========================================================================
      console.log('\n✅ ROSTER ASSIGNMENT COMPLETED');
      console.log('='.repeat(80) + '\n');
      ...
      res.json({
        ...
        data: {
          ...updatedRoster,
          distanceData: distanceData && !distanceData.error ? distanceData : null
        }
      });
    } catch (err) {
      console.error('❌ FATAL ERROR IN ROSTER ASSIGNMENT:', err);
      res.status(500).json({
        success: false,
...
router.put('/admin/edit-assignment/:id',
  ...
  async (req, res) => {
    console.log('\n' + '='.repeat(80));
    console.log('🔄 ROSTER ASSIGNMENT UPDATE INITIATED');
    console.log('='.repeat(80));
    console.log('📅 Timestamp:', new Date().toISOString());
    console.log('👤 Admin User ID:', req.user.userId);
    ...
    try {
      ...
      const rosterObjectId = new ObjectId(rosterId);
      ...
      if (ObjectId.isValid(driverId)) driverQuery.$or.push({ _id: new ObjectId(driverId) });
      ...
      if (ObjectId.isValid(vehicleId)) vehicleQuery.$or.push({ _id: new ObjectId(vehicleId) });
      ...
      // ✅ REVERSE GEOCODE LOCATIONS (NEW - IMPROVED)
      console.log('\n🌍 Reverse geocoding locations...');

      let pickupAddress = 'Not specified';
      let dropAddress = 'Not specified';

      // Helper function for reverse geocoding - returns only readable address
      const reverseGeocode = async (lat, lng) => {
        try {
          ...
          if (data && data.address) {
            // Build a clean address without coordinates
            const address = data.address;
            const parts = [];
      ...
      if (roster.locations?.pickup?.coordinates) {
        ...
        if (roster.locations.pickup.address &&
          !roster.locations.pickup.address.includes(',') &&
          roster.locations.pickup.address.length < 50) {
          // If existing address looks like coordinates or is too short, reverse geocode
        ...
        const coords = roster.locations.loginPickup.coordinates;
        if (roster.locations.loginPickup.address &&
          !roster.locations.loginPickup.address.includes(',') &&
          roster.locations.loginPickup.address.length < 50) {
          pickupAddress = await reverseGeocode(coords.latitude, coords.longitude);
        } else {
          pickupAddress = roster.locations.loginPickup.address ||
            await reverseGeocode(coords.latitude, coords.longitude);
        }
      }
      ...
      if (roster.locations?.drop?.coordinates) {
        ...
        if (roster.locations.drop.address &&
          !roster.locations.drop.address.includes(',') &&
          roster.locations.drop.address.length < 50) {
        ...
        const coords = roster.locations.logoutDrop.coordinates;
        if (roster.locations.logoutDrop.address &&
          !roster.locations.logoutDrop.address.includes(',') &&
          roster.locations.logoutDrop.address.length < 50) {
          dropAddress = await reverseGeocode(coords.latitude, coords.longitude);
        } else {
          dropAddress = roster.locations.logoutDrop.address ||
            await reverseGeocode(coords.latitude, coords.longitude);
        }
      }
      ...
      const finalDoc = updatedDoc && updatedDoc._id ? updatedDoc :
        (await req.db.collection('rosters').findOne({ _id: rosterObjectId }));
      ...
      const formattedStartDate = new Date(roster.startDate).toLocaleDateString('en-IN', {
        day: '2-digit',
        month: 'short',
        year: 'numeric'
      });
      const formattedEndDate = new Date(roster.endDate).toLocaleDateString('en-IN', {
        day: '2-digit',
        month: 'short',
        year: 'numeric'
      });

      const rosterTypeText = roster.rosterType === 'both' ? 'Login & Logout' :
        roster.rosterType === 'login' ? 'Login Only' : 'Logout Only';

      // ✅ SEND CUSTOMER NOTIFICATION (NO SYMBOLS, CLEAN FORMAT)
      ...
      try {
        ...
        const existingNotification = await req.db.collection('notifications').findOne({
          ...
          'metadata.action': 'roster_assignment_updated',
        ...
        if (existingNotification) {
          ...
          const customerNotificationBody = `Driver and Vehicle Assigned

Office: ${roster.officeLocation}
Type: ${rosterTypeText}
Date: ${formattedStartDate} to ${formattedEndDate}
Time: ${roster.startTime} - ${roster.endTime}
...
Drop Location: ${dropAddress}`.trim();
          ...
          await createNotification(req.db, {
            ...
            data: {
              ...
              timeRange: `${roster.startTime} - ${roster.endTime}`,
              driverName: driverName,
            ...
            priority: 'high',
            category: 'roster',
            style: 'success' // Green background indicator
          });
      ...
      // ✅ SEND DRIVER NOTIFICATION (NO SYMBOLS, CLEAN FORMAT)
      console.log('\n📧 Sending driver notification...');
      ...
      try {
        ...
        if (driverUser && driverUser.firebaseUid) {
          ...
          const existingDriverNotification = await req.db.collection('notifications').findOne({
            userId: driverUser.firebaseUid,
            'metadata.rosterId': rosterId,
            'metadata.action': 'driver_assigned_to_roster',
            createdAt: { $gte: new Date(Date.now() - 60000) } // Within last 1 minute
          });

          if (existingDriverNotification) {
            console.log('⚠️  Driver notification already sent, skipping duplicate');
          } else {
            const driverNotificationBody = `New Roster Assignment
...
Drop Location: ${dropAddress}`.trim();
      ...
      res.json({
        ...
        data: {
          rosterId: finalDoc._id.toString(),
          status: finalDoc.status,
          assignedDriver: {
...
router.post('/admin/assign-batch', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Step 2: Check vehicle capacity
    console.log('\n📊 Step 2: Checking vehicle capacity...');
    const totalSeats = vehicle.seatCapacity || 4;
    const assignedCustomers = vehicle.assignedCustomers || [];
    const availableSeats = totalSeats - 1 - assignedCustomers.length; // -1 for driver

    console.log(`   Total Seats: ${totalSeats}`);
    console.log(`   Assigned: ${assignedCustomers.length}`);
    console.log(`   Available: ${availableSeats}`);
    console.log(`   Requested: ${rosterIds.length}`);
    ...
    // Step 4: Update vehicle assigned customers
    console.log('\n🚗 Step 4: Updating vehicle capacity...');
    const newAssignedCustomers = [...assignedCustomers, ...rosterIds];
    ...
    console.log(`✅ Vehicle now has ${newAssignedCustomers.length} assigned customers`);
    ...
    res.json({
      ...
      data: {
        ...
        vehicle: {
          ...
          availableSeats: totalSeats - 1 - newAssignedCustomers.length
        }
      }
    });
