  ...
  try {
    ...
    let driver = await req.db.collection('drivers').findOne({
      $or: [
        ...
        { email: user.userId  },                     // Legacy: UID field
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const newDriver = { email: firebaseUid, // ← FIREBASE UID ADDED HERE
      driverId,
  ...
  try {
    ...
    for (let i = 0; i < drivers.length; i++) {
      ...
      try {
        ...
        const newDriver = { email: firebaseUid,
          driverId,
